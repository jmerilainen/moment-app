  ...
  return (
    <div className="flex gap-[0.5em] group relative">
      <button type="button" onClick={() => onToggleDone()}>
        <span
          className={`${
            done ? "bg-current" : "hover:bg-white/10"
          } flex w-[0.75em] h-[0.75em] border border-current rounded-full  transition`}
        ></span>
        {done ? (
      ...
      <div className="absolute right-0">
        <button
          className="transition opacity-0 group-hover:opacity-60 focus:opacity-60"
          onClick={() => onDelete()}
        >
...
function Day({
  ...
  return (
    <div className="relative p-4 font-sans text-xs ">
      <div className="grid gap-4">
        <div className="md:text-right">
          <span
            className={`${
              isToday
                ? "bg-black text-white w-[1.5em] h-[1.5em] inline-flex items-center justify-center rounded-full"
                : ""
            }`}
          >
            {format(date, "d")}
...
export default function Calendar({
  ...
  return <CalendarGrid date={date} day={(day) => <DayForDate date={day} />} />;
}
